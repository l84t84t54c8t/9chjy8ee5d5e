...
def get_command(lang: str = "ckb") -> Union[str, List[str]]:
...
commands["ckb"] = load_yaml_file(r"./strings/cmds/ckb.yml")
english_keys = set(commands["ckb"].keys())

for filename in os.listdir(r"./strings/cmds/"):
    if filename.endswith(".yml") and filename != "ckb.yml":
...
for filename in os.listdir(r"./strings/langs/"):
    if filename.endswith(".yml") and filename != "en.yml":
        ...
        try:
            ...
            print(
                "There is an issue with the language file. Please report it to Mohammed at @MGIMT on Telegram"
            )
            sys.exit()

if not commands:
    print(
        "There's a problem loading the command files. Please report it to Mohammed at @MGIMT on Telegram"
    )
    sys.exit()
